...
class Users(db.Model):
    ...
    @classmethod
    def delete_user_by_email(cls, _email):
        try:
            cls.query.filter_by(email=_email).delete()
            db.session.commit()
        except:
            return False
    ...
    def get_users_by_filter(cls, filter):
        query = "select * from users inner join user_details on Users.id = UserDetails.user where "
        for key, value in filter.items():
            query = "{} UserDetails.{} = {}".format(query, key, value)
        print(query)
        result = db.engine.execute(query)
        list_result = []
        for job in result:
