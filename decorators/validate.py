...
def role(func):
    """
    The function should validate the user role assignment request
    :param func:
    :return: 400 and the error text
    """
    @wraps(func)
    def wrapper(*args, **kwargs):
        body = request.get_json()
        # role should be defined for the user
        if "role" not in body:
            error = {
                "status": "failure",
                "message": "Bad Input, Missing role attribute."
            }
            return Response(dumps(error), 400, mimetype="application/json")
        if "role" in body and not body["role"]:
            error = {
                "status": "failure",
                "message": "Bad Input, Please enter role for the user"
            }
            return Response(dumps(error), 400, mimetype="application/json")
        return func(*args, **kwargs)
    return wrapper
